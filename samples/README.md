...
Use these files for a **first run** without hunting for a script. ScriptRAG only ingests **`.fdx`**; the **`.pdf`** files are human-readable companions (export a prettier PDF anytime from Final Draft).
...
```bash
cp samples/cinema-four/Cinema_Four.fdx target_script.fdx
# or
cp samples/ludwig/Ludwig.fdx target_script.fdx
```

See the root [**README**](../README.md) **Demo** section for the full walkthrough.
